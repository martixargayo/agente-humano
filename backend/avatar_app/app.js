...
function buildMouthPointsGeometryFromAnimatedSurface(srcGeometry) {
  ...
  if (!basePosAttr || !mouthWeightAttr || !mouthSideAttr || !headWeightAttr) {
    console.warn('[mouth-points] Faltan atributos requeridos para construir la geometría de boca.');
    return { geometry: null, pointCount: 0, upsampled: false, sourceCount: 0 };
  }
  ...
  const hw = [];
  const ov = [];
  const addPoint = (x, y, z, mouthWeight, mouthSide, headWeight, overlay) => {
    p.push(x, y, z);
    b.push(x, y, z);
    mw.push(mouthWeight);
    ms.push(mouthSide);
  ...
  for (let i = 0; i < basePosAttr.count; i++) {
    ...
    addPoint(x, y, z, w, mouthSideAttr.getX(i), headWeightAttr.getX(i), overlay);
  }

  const sourceCount = p.length / 3;
  let upsampled = false;
  if (sourceCount > 0 && sourceCount < tuning.upsampleMinPoints) {
    upsampled = true;
    for (let i = 0; i < sourceCount; i++) {
      ...
      const y = p[idx + 1];
      const z = p[idx + 2];
      const h1 = stableHash3(x, y, z, 1.0);
      const h2 = stableHash3(x, y, z, 2.0);
      const angle = h1 * Math.PI * 2.0;
      const amp = tuning.upsampleAmpMin + (tuning.upsampleAmpMax - tuning.upsampleAmpMin) * h2;
      ...
      const headW = hw[i];
      const overlay = ov[i];
      addPoint(x + dx, y + dy, z, w, s, headW, overlay);
    }
  }
...
void main() {
  ...
  float mouthFade = clamp(uMouthOpenVisual * uMouthMeshFade * innerFadeMask, 0.0, 0.98);
  finalColor = mix(finalColor, finalColor * 0.9, mouthFade * 0.25);
  ...
  if (uDebugMouthFade > 0.5) {
    ...
    gl_FragColor = vec4(dbg, 1.0 - mouthFade * 0.75);
    return;
  }

  gl_FragColor = vec4(finalColor, 1.0 - mouthFade * 0.75);
}
`;
      ...
      if (mouthBuild.geometry && mouthBuild.pointCount > 0) {
        ...
        if (DEBUG_MOUTH_POINTS_ENABLED) {
          console.info('[mouth-points] built', {
            ...
            upsampled: mouthBuild.upsampled,
          });
        }
      }
    } else {
