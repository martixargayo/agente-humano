...
void main() {
  ...
  // Eliminamos el recorte geométrico tipo "rombo" y lo sustituimos por un cull orgánico
  // basado en la máscara de boca (vMouthWeight), manteniendo el mismo gating temporal.
  float mouthHoleActive = step(0.5, uMouthHoleActive) * step(0.5, uUseDiamondFade);
  float holeInner = smoothstep(0.70, 0.92, vMouthWeight);
  float holeOpen = clamp((uMouthOpenVisual - 0.02) / 0.22, 0.0, 1.0);
  float holeMask = mouthHoleActive * holeInner * holeOpen;
  float holeNoise = fract(sin(dot(vBaseXY * 47.31 + vUv * 11.29, vec2(12.9898, 78.233))) * 43758.5453123);
  float holeThreshold = smoothstep(0.35, 0.9, holeMask);
  if (holeThreshold > 0.001 && holeNoise < holeThreshold) {
    discard;
  }
  ...
  if (uDebugMouthFade > 0.5 || uDebugMouthDiamond > 0.5) {
    ...
    if (uDebugMouthDiamond > 0.5) {
      dbg = mix(dbg, vec3(1.0, 0.0, 1.0), clamp(holeMask, 0.0, 0.8));
    }
