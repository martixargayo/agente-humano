...
export function createDemoFeedbackMode({
  ...
  function ensureFeedbackOverlay() {
    ...
    for (const section of FEEDBACK_CARDS) {
      ...
      card.className = `fb-card fb-skill-card fb-skill-card-${grid.children.length + 1} ${section.status}`;
  ...
  function ensureFeedbackStyles() {
    ...
    style.textContent = `
      ...
      .demo-feedback-overlay::before {
        ...
        pointer-events: none;
        background:
          radial-gradient(420px 320px at 88% 6%, rgba(46,144,250,0.10), transparent 70%),
          radial-gradient(440px 320px at 10% 92%, rgba(18,183,106,0.08), transparent 72%),
          radial-gradient(360px 260px at 62% 58%, rgba(247,144,9,0.045), transparent 74%),
          radial-gradient(rgba(11,15,20,1) 0.5px, transparent 0.5px);
      ...
      .demo-feedback-dashboard::before {
        content: '';
        position: absolute;
        left: 28px;
        right: 28px;
        ...
        bottom: 22px;
        background:
          linear-gradient(180deg, transparent, rgba(102,112,133,0.16), transparent) 0 5%/1px 120px no-repeat,
          linear-gradient(180deg, transparent, rgba(102,112,133,0.12), transparent) 100% 30%/1px 140px no-repeat;
        pointer-events: none;
      }

      .fb-card {
        background: rgba(255,255,255,0.72);
        border: 1px solid rgba(183,191,203,0.56);
        position: relative;
        border-radius: var(--radius-lg);
        box-shadow: 0 24px 60px rgba(11,15,20,0.08), 0 6px 14px rgba(11,15,20,0.06);
      ...
      .fb-card::before {
        ...
        width: 54%;
        height: 68px;
        background: linear-gradient(130deg, rgba(255,255,255,0.44), rgba(255,255,255,0));
      ...
      .fb-card:hover {
        ...
        box-shadow: 0 26px 62px rgba(11,15,20,0.10), 0 8px 16px rgba(11,15,20,0.08);
      }
      ...
      .fb-header::after {
        ...
        bottom: 10px;
        height: 1px;
        background: linear-gradient(90deg, transparent, rgba(46,144,250,0.3), transparent);
      }
      ...
      .fb-score-pill {
        ...
        background: linear-gradient(rgba(255,255,255,0.78), rgba(255,255,255,0.72)) padding-box, linear-gradient(120deg, rgba(46,144,250,0.42), rgba(18,183,106,0.35)) border-box;
        border-radius: 999px;
        padding: 7px 10px;
        font-size: 16px;
        font-weight: 600;
      ...
      .fb-skill-card.ok::before { background: linear-gradient(180deg, rgba(18,183,106,0.2), var(--ok), rgba(18,183,106,0.18)); }
      .fb-skill-card.warn::before { background: linear-gradient(180deg, rgba(247,144,9,0.2), var(--warn), rgba(247,144,9,0.18)); }
      .fb-skill-card.bad::before { background: linear-gradient(180deg, rgba(240,68,56,0.2), var(--bad), rgba(240,68,56,0.18)); }

      .fb-skill-card-1::after { background-image: radial-gradient(circle, rgba(102,112,133,1) 1px, transparent 1px); }
      .fb-skill-card-2::after { background-image: linear-gradient(90deg, rgba(102,112,133,1) 1px, transparent 1px); background-size: 7px 7px; }
      .fb-skill-card-3::after { background-image: linear-gradient(135deg, rgba(102,112,133,1) 1px, transparent 1px); }
      .fb-skill-card-4::after { background-image: linear-gradient(0deg, rgba(102,112,133,1) 1px, transparent 1px); background-size: 8px 8px; }
      ...
      .fb-badge::before { content: '•'; font-size: 12px; line-height: 1; }

      .fb-badge.ok { background: var(--okBg); color: #0e9f60; box-shadow: inset 0 0 8px rgba(18,183,106,0.13); }
      .fb-badge.warn { background: var(--warnBg); color: #d97b03; box-shadow: inset 0 0 8px rgba(247,144,9,0.13); }
      .fb-badge.bad { background: var(--badBg); color: #dd3c30; box-shadow: inset 0 0 8px rgba(240,68,56,0.12); }
      ...
      .fb-chart-shell { height: 240px; width: 100%; position: relative; }
      .fb-chart { width: 100%; height: 240px; display: block; }

      .fb-chart-shell::after {
        content: '';
        position: absolute;
        left: 6px;
        right: 6px;
      ...
      .fb-detail-head .meta { color: var(--muted); font-weight: 500; }
      ...
      .fb-quote {
        ...
        border-left: 2px solid rgba(46,144,250,0.32);
        border-radius: 12px;
        padding: 10px;
        font-size: 12.5px;
        margin: 0;
...
function renderDetailPanel(panel, index) {
  ...
  panel.innerHTML = `
    <div class="fb-detail-head"><span class="fb-chip">Turno ${turn}</span><span class="fb-chip meta">Score ${score}</span><span class="fb-chip delta ${deltaClass}">(${deltaText})</span></div>
...
function renderChart(svg, onSelectTurn, getSelectedIndex) {
  ...
  defs.innerHTML = '<filter id="fbPointShadow" x="-50%" y="-50%" width="200%" height="200%"><feDropShadow dx="0" dy="5" stdDeviation="3.5" flood-color="rgba(11,15,20,0.12)"/></filter><pattern id="fbZoneGrain" width="8" height="8" patternUnits="userSpaceOnUse"><circle cx="1" cy="1" r="0.6" fill="rgba(18,183,106,0.12)"/></pattern>';
  ...
  const acceptancePlate = document.createElementNS(NS, 'rect');
  acceptancePlate.setAttribute('x', String(pad.left));
  acceptancePlate.setAttribute('y', String(yFor(100)));
  acceptancePlate.setAttribute('width', String(innerW));
  acceptancePlate.setAttribute('height', String(yFor(80) - yFor(100)));
  acceptancePlate.setAttribute('fill', 'url(#fbZoneGrain)');
  acceptancePlate.setAttribute('opacity', '0.22');
  svg.appendChild(acceptancePlate);
  const acceptanceTop = document.createElementNS(NS, 'line');
  acceptanceTop.setAttribute('x1', String(pad.left));
  acceptanceTop.setAttribute('x2', String(width - pad.right));
  acceptanceTop.setAttribute('y1', String(yFor(80)));
  acceptanceTop.setAttribute('y2', String(yFor(80)));
  acceptanceTop.setAttribute('stroke', 'rgba(18,183,106,0.28)');
  acceptanceTop.setAttribute('stroke-width', '1');
  svg.appendChild(acceptanceTop);

  for (let i = 0; i < TURN_SCORES.length; i += 1) {
    const tick = document.createElementNS(NS, 'line');
    const tx = xFor(i);
    tick.setAttribute('x1', String(tx));
    tick.setAttribute('x2', String(tx));
    tick.setAttribute('y1', String(height - pad.bottom + 1));
    tick.setAttribute('y2', String(height - pad.bottom + (i % 2 === 0 ? 6 : 4)));
    tick.setAttribute('stroke', 'rgba(102,112,133,0.22)');
  ...
  const guideTick = document.createElementNS(NS, 'line');
  guideTick.setAttribute('x1', String(xFor(0) - 4));
  guideTick.setAttribute('x2', String(xFor(0) + 4));
  guideTick.setAttribute('y1', String(pad.top + 1));
  guideTick.setAttribute('y2', String(pad.top + 1));
  guideTick.setAttribute('stroke', 'rgba(46,144,250,0.45)');
  guideTick.setAttribute('stroke-width', '1');
  guideTick.style.display = 'none';
  guideTick.classList.add('fb-guide-tick');
  svg.appendChild(guideTick);
  ...
  TURN_SCORES.forEach((value, idx) => {
    ...
    highlight.setAttribute('cx', String(x - 1.4));
    highlight.setAttribute('cy', String(y - 1.6));
    highlight.setAttribute('r', '1.4');
    highlight.setAttribute('fill', 'rgba(255,255,255,0.72)');
    highlight.setAttribute('pointer-events', 'none');
    highlight.classList.add('fb-point-highlight');
    pointLayer.appendChild(highlight);
  });
  ...
  function drawSegment(container, x1, y1, x2, y2, v1, v2) {
    ...
    if (s1 === s2 || (v1 < 80 && v2 < 80 && (v1 >= 60 || v2 >= 60))) {
      ...
      glow.setAttribute('stroke', colorFor(s2));
      glow.setAttribute('stroke-width', '4.2');
      glow.setAttribute('stroke-opacity', s2 === 'ok' ? '0.23' : '0.12');
...
function updateChartSelection(svg, index) {
  ...
  const guideTick = svg.querySelector('.fb-guide-tick');

  points.forEach((p, idx) => {
    p.setAttribute('r', idx === index ? '7.2' : '5');
    p.style.transition = 'r 150ms ease-out, opacity 150ms ease-out';
  });
  ...
  if (guide && index !== null && index !== undefined) {
    ...
    if (guideTick) {
      guideTick.setAttribute('x1', String(Number(x) - 4));
      guideTick.setAttribute('x2', String(Number(x) + 4));
      guideTick.style.display = 'block';
    }
  } else if (guideTick) {
    guideTick.style.display = 'none';
  }
...
function updateScrubberUI(dashboard, value) {
  ...
  dots.forEach((dot, idx) => {
    ...
    dot.style.transform = idx + 1 === value ? 'scale(1.35)' : 'scale(1)';
  });
}
