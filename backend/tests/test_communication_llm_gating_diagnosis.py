...
class CommunicationLlmGatingDiagnosisTests(unittest.TestCase):
    ...
    def test_content_safe_mode_sets_disabled_policy(self) -> None:
        with patch.dict(os.environ, {'COMMUNICATION_FORCE_SAFE_MODE': 'true'}, clear=False):
        ...
        self.assertEqual(output['runtime_meta']['reason'], 'disabled_policy')

    def test_content_llm_without_api_key_falls_back_missing_openai_api_key(self) -> None:
        with patch.dict(os.environ, {'OPENAI_API_KEY': '', 'COMMUNICATION_FORCE_SAFE_MODE': 'false'}, clear=False):
            output = evaluate_content_from_transcript(bundle=self._bundle_for_content())
        self.assertEqual(output['runtime_meta']['mode'], 'fallback')
        self.assertEqual(output['runtime_meta']['reason'], 'missing_openai_api_key')

    def test_content_llm_with_mocked_llm_sets_llm_mode(self) -> None:
        ...
        with (
            patch.dict(os.environ, {'OPENAI_API_KEY': 'test-key', 'COMMUNICATION_FORCE_SAFE_MODE': 'false'}, clear=False),
    ...
    def test_delivery_safe_mode_sets_disabled_policy(self) -> None:
        with patch.dict(os.environ, {'COMMUNICATION_FORCE_SAFE_MODE': 'true'}, clear=False):
        ...
        self.assertEqual(runtime_meta['reason'], 'disabled_policy')

    def test_delivery_llm_without_api_key_falls_back_missing_openai_api_key(self) -> None:
        with patch.dict(os.environ, {'OPENAI_API_KEY': '', 'COMMUNICATION_FORCE_SAFE_MODE': 'false'}, clear=False):
            _, _, runtime_meta = evaluate_delivery_with_specialized_from_audio_metrics(audio_features=self._audio_features_ready(), transcript_excerpt='x')
        self.assertEqual(runtime_meta['mode'], 'fallback')
        self.assertEqual(runtime_meta['reason'], 'missing_openai_api_key')

    def test_visual_default_mode_is_llm_v1(self) -> None:
        with patch.dict(os.environ, {'COMMUNICATION_FORCE_SAFE_MODE': 'false'}, clear=False), patch(
            'evaluacion.engine.communication_evaluators.evaluate_visual_llm_v1_from_features'
        ) as run_visual:
            run_visual.return_value = (
                CommunicationVisualEvaluationV1(
                    score_0_100=80,
                ...
                [],
                CommunicationSpecializedDimensionEvalV1(score_1_5=4, label='medio-alto', reason_short='ok'),
            )
            output = evaluate_communication_visual(self._bundle_for_visual())
        self.assertEqual(output['visual_mode'], 'llm_v1')
        self.assertEqual(output['runtime_meta']['mode'], 'llm')

    def test_visual_safe_mode_forces_metadata(self) -> None:
        with patch.dict(os.environ, {'COMMUNICATION_FORCE_SAFE_MODE': 'true'}, clear=False):
            output = evaluate_communication_visual(self._bundle_for_visual())
        self.assertEqual(output['runtime_meta']['mode'], 'metadata')
        self.assertEqual(output['runtime_meta']['reason'], 'disabled_policy')

    def test_synthesis_safe_mode_sets_disabled_policy(self) -> None:
    ...
    def test_synthesis_llm_without_api_key_returns_missing_key_reason(self) -> None:
        synthesis_input = build_global_synthesis_input(
    ...
    def test_synthesis_llm_with_mocked_llm_sets_llm_mode(self) -> None:
        synthesis_input = build_global_synthesis_input(
    ...
    def test_activation_matrix_safe_mode_vs_llm_mode_vs_missing_key(self) -> None:
        ...
        self.assertEqual(content['runtime_meta']['reason'], 'disabled_policy')
        self.assertEqual(delivery['runtime_meta']['mode'], 'fallback')
        self.assertEqual(delivery['runtime_meta']['reason'], 'disabled_policy')
        self.assertEqual(visual['runtime_meta']['mode'], 'metadata')
        self.assertEqual(visual['runtime_meta']['reason'], 'disabled_policy')
        self.assertEqual(synthesis_meta['mode'], 'fallback')
        self.assertEqual(synthesis_meta['fallback_reason'], 'disabled_policy')

        # Caso B: policy code (LLM ON) + API key + LLM mock
        ...
        # Caso C: policy code (LLM ON) sin API key => razones explícitas
        from evaluacion.engine.communication_visual_openai_client import CommunicationVisualLlmError

        with (
            patch.dict(os.environ, {'OPENAI_API_KEY': '', 'COMMUNICATION_FORCE_SAFE_MODE': 'false'}, clear=False),
            patch(
                'evaluacion.engine.communication_evaluators.evaluate_visual_llm_v1_from_features',
