...
def policy_progress_node(state: dict) -> dict:
    ...
    planner_judge_enabled = os.getenv("POLICY_PLAN_JUDGE_ENABLED", "0") == "1"
    if planner_judge_enabled:
    ...
    policy_state, policy_hint, policy_meta = update_policy_state(
        ...
        policy_plan_judgement=policy_plan_judgement if planner_judge_enabled else None,
        active_plan_status=str(progress_state.get("active_plan_status", "none") or "none"),
        judgement_missing_streak=int(progress_state.get("judgement_missing_streak", 0) or 0) if planner_judge_enabled else 0,
    )
