...
def phase_policy_planner_node(state: dict) -> dict:
    ...
    if not previous_plan:
        ...
        if judgement_skip_planner:
            ...
            steps = list((previous_plan or {}).get("steps", [])) if isinstance(previous_plan, dict) else []
            step_idx = int((previous_plan or {}).get("current_step_idx", 0) or 0) if isinstance(previous_plan, dict) else 0
